...
/**
 * Current database version: 'jp' or 'en'
 */
let currentDb = 'jp';

/**
 * Get the current database version
 * @returns {string} 'jp' or 'en'
 */
...
/**
 * Load race data for the specified database version
 * @param {string} db - 'jp' or 'en'
 * @returns {Array} Array of race objects
 */
export function loadRaceData(db) {
    if (db) currentDb = db;
    
    const source = currentDb === 'en'
        ? (typeof window !== 'undefined' && window.RACES_EN)
        : (typeof window !== 'undefined' && window.RACES);
    
    if (source && Array.isArray(source)) {
        updateDataSourceIndicator(true, currentDb);
        return source;
    }
 ...
 * @param {string} db - Which database is active
 */
function updateDataSourceIndicator(hasData = true, db = 'jp') {
    const ind = document.getElementById('data-source-indicator');
    if (!ind) return;
    
    if (hasData) {
        const meta = db === 'en'
            ? (typeof window !== 'undefined' && window.RACES_EN_META ? window.RACES_EN_META : null)
            : (typeof window !== 'undefined' && window.RACES_META ? window.RACES_META : null);
        ...
        const label = db === 'en' ? 'races-en.js (Global)' : 'races.js (JP)';
        ind.textContent = `Data source: ${label} — generated ${dateText}${countText}`;
    } else {
 ...
 * @param {string} [db] - Optional database version to load ('jp' or 'en')
 */
export function initializeRaceData(db) {
    state.races = loadRaceData(db);
...
/**
 * Switch race database and re-initialize state.
 * Returns the new db version for the caller to trigger re-renders.
 * @param {string} db - 'jp' or 'en'
 * @returns {string} The active database version
 */
export function switchDatabase(db) {
    if (db !== 'jp' && db !== 'en') return currentDb;
    initializeRaceData(db);
    return currentDb;
}
