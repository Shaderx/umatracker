    ...
    attachPickerSwipeHandlers() {
        ...
        const onTouchMove = (e) => {
            ...
            if (isDragging) {
                ...
                carousel.style.transform = `translate3d(${baseTranslate + dragPercent}%, 0, 0)`;
            }
        };
        ...
        const onTouchEnd = (e) => {
            ...
            if (isDragging && (Math.abs(dx) > 80 || velocity > 0.3) && Math.abs(dx) > dy) {
                ...
                carousel.style.transform = `translate3d(${baseTranslate}%, 0, 0)`;
            }
		...
		navigatePickerWithAnimation(direction) {
			const carousel = document.getElementById('picker-carousel');
			if (!carousel) return;
			this.updateToggleButton(); // pre-sync toggle state on all cards
			carousel.style.transition = 'transform 260ms cubic-bezier(0.25, 0.46, 0.45, 0.94)';
			const targetTranslate = direction > 0 ? '-66.666%' : '0%';
			carousel.style.transform = `translate3d(${targetTranslate}, 0, 0)`;
			setTimeout(() => {
				carousel.classList.add('no-transition');
				carousel.style.transition = 'none';
